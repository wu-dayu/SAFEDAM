...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def _prepare_memory_conditioned_features_legacy(
        self,
        frame_idx,
        ...
        """
        Legacy D4SM memory path.
        Supports wrapper state with:
            - per_obj_dict
        ...
        obj_keys = output_dict["obj_ids_list"]
        if len(obj_keys) == 0:
        ...
        for obj_id in obj_keys:
            mem_list = output_dict["per_obj_dict"][obj_id]
            ptr_list = output_dict["per_obj_obj_ptr_dict"][obj_id]

            mem_feats = []
            mem_pos = []

            for idx_ in range(len(mem_list)):
                mem_el = mem_list[idx_]
                mem_feats.append(mem_el["maskmem_features"])

                if mem_el["is_init"] or mem_el["is_drm"]:
                    maskmem_tpos_enc = self.maskmem_tpos_enc[-1]
                else:
                    tpos_enc_idx = len(mem_list) - idx_ - 1
                    tpos_enc_idx = min(tpos_enc_idx, self.num_maskmem - 1)
                    maskmem_tpos_enc = self.maskmem_tpos_enc[tpos_enc_idx]

                mem_pos.append(
                    output_dict["maskmem_pos_enc"]
                    + maskmem_tpos_enc.permute(2, 0, 1).unsqueeze(0)
                )

            ptr_feats = []
            rel_pos = []
            for idx_ in range(len(ptr_list)):
                ptr_feats.append(ptr_list[idx_]["obj_ptr"])
                if idx_ == 0:
                    rel_pos.append(frame_idx)
                else:
                    rel_pos.append(len(ptr_list) - idx_)

            rel_pos = self._get_tpos_enc(
                rel_pos,
                device=device,
                max_abs_pos=max(2, len(ptr_list) - 1),
            )

            per_obj_memory.append(torch.cat(mem_feats, 0))
            per_obj_memory_pos.append(torch.cat(mem_pos, 0))
            per_obj_obj_ptrs.append(torch.cat(ptr_feats, 0))
            per_obj_obj_pos.append(rel_pos)

        # Group objects that have the same prompt shape so we can batch encoder calls safely
        grouped = {}
        for obj_idx, obj_id in enumerate(obj_keys):
            memory_ = per_obj_memory[obj_idx].flatten(2).permute(2, 0, 1)
            n_spatial_tokens = memory_.shape[0] * memory_.shape[1]

            if self.mem_dim < C:
                obj_ptrs_ = per_obj_obj_ptrs[obj_idx].reshape(
                    -1, C // self.mem_dim, self.mem_dim
                )
                num_obj_ptr_tokens = obj_ptrs_.shape[0] * obj_ptrs_.shape[1]
            else:
                raise NotImplementedError(
                    "Legacy D4SM path expects self.mem_dim < self.hidden_dim."
                )

            key = (n_spatial_tokens + num_obj_ptr_tokens, num_obj_ptr_tokens)
            grouped.setdefault(key, []).append((obj_idx, obj_id))

        per_obj_pix_feat_with_mem = [None] * len(obj_keys)

        for (_, num_obj_ptr_tokens), group_items in grouped.items():
            multi_prompt = []
            multi_prompt_pos = []

            for obj_idx, obj_id in group_items:
                memory_ = per_obj_memory[obj_idx].flatten(2).permute(2, 0, 1)
                memory_pos_ = per_obj_memory_pos[obj_idx].flatten(2).permute(2, 0, 1)

                obj_ptrs_ = per_obj_obj_ptrs[obj_idx].reshape(
                    -1, C // self.mem_dim, self.mem_dim
                )
                obj_pos_ = per_obj_obj_pos[obj_idx].unsqueeze(1).repeat_interleave(
                    C // self.mem_dim, dim=1
                )

                prompt_ = torch.cat(
                    [
                        memory_.permute(1, 0, 2).flatten(0, 1),
                        obj_ptrs_.flatten(0, 1),
                    ],
                    dim=0,
                ).unsqueeze(1)

                prompt_pos_ = torch.cat(
                    [
                        memory_pos_.permute(1, 0, 2).flatten(0, 1),
                        obj_pos_.flatten(0, 1),
                    ],
                    dim=0,
                ).unsqueeze(1)

                multi_prompt.append(prompt_)
                multi_prompt_pos.append(prompt_pos_)

            group_B = len(group_items)
            curr_feats = [current_vision_feats[0][:, 0:1, :].expand(-1, group_B, -1)]
            curr_pos = [current_vision_pos_embeds[0][:, 0:1, :].expand(-1, group_B, -1)]

            encoder_out = self.transformer.encoder(
                src=curr_feats,
                src_key_padding_mask=[None],
                src_pos=curr_pos,
                prompt=torch.cat(multi_prompt, dim=1),
                prompt_pos=torch.cat(multi_prompt_pos, dim=1),
                prompt_key_padding_mask=None,
                feat_sizes=feat_sizes,
                num_obj_ptr_tokens=num_obj_ptr_tokens,
            )

            pix_feat_with_mem = encoder_out["memory"].permute(1, 2, 0).view(group_B, C, H, W)

            for i, (obj_idx, _) in enumerate(group_items):
                per_obj_pix_feat_with_mem[obj_idx] = pix_feat_with_mem[i : i + 1]

        return torch.cat(per_obj_pix_feat_with_mem, dim=0)
    ...
    def _prepare_memory_conditioned_features(
        ...
        if (
            ...
            and use_prev_mem_frame
            and "cond_frame_outputs" not in output_dict
            and "per_obj_dict" in output_dict
        ):
            return self._prepare_memory_conditioned_features_legacy(
                frame_idx=frame_idx,
    ...
    def track_step(
        ...
        current_out["multimasks_logits"] = high_res_multimasks
        if self.use_memory_selection:
