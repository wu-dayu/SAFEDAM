...
class DAM4SAMMOT():
    ...
    def track(self, image):
        ...
        selected_mask_is_alternative_by_obj_idx = [False for _ in range(len(self.all_obj_ids))]
        chosen_mask_idx_by_obj_idx = [int(np.argmax(all_ious[obj_idx])) for obj_idx in range(len(self.all_obj_ids))]
        for obj_idx in range(len(self.all_obj_ids)):
            ...
            if (
                lock_choice["candidate_idx"] is not None
                and lock_choice["mask"] is not None
                and lock_choice["candidate_idx"] != chosen_mask_idx_by_obj_idx[obj_idx]
            ):
                selected_mask_is_alternative_by_obj_idx[obj_idx] = True
        ...
        for obj_idx, obj_id in enumerate(self.all_obj_ids):
            ...
            chosen_mask_idx = chosen_mask_idx_by_obj_idx[obj_idx]
            ...
            if self.overlap_update_freeze_state.get(obj_id, False):
                ...
                update_gate = False
            elif selected_mask_is_alternative_by_obj_idx[obj_idx]:
                self.overlap_update_freeze_state[obj_id] = False
            ...
            if update_gate:
                ...
                if self.max_drm > 0:
                    ...
                    m_idx = int(lock_choice["candidate_idx"])
